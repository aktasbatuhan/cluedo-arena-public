...
export class Game extends EventEmitter {
  ...
  async updateAgentMemories(suggestingAgent, suggestion, challengeResult, accusationDecision) {
      ...
      for (const agent of this.agents) {
          ...
          // CAPTURE PRIOR KNOWLEDGE (eliminated cards before this turn's events impact memory)
          const eliminatedCardsBeforeThisTurnEvent = new Set(agent.memory.eliminatedCards);

          this.logger.info(`Processing memory update for agent: ${agent.name}`);
          this.logger.debug(`Agent cards: ${Array.from(agent.cards).join(', ')}`);
          // Log the state *before* this turn's potential additions
          this.logger.debug(`Agent eliminated cards (before this turn's events): ${Array.from(eliminatedCardsBeforeThisTurnEvent).join(', ')}`);
          ...
          try {
              ...
              if (challengeResult && challengeResult.cardToShow) {
                  if (agent.name === suggestingAgent.name) {
                      ...
                      // OBSERVERS DO NOT SEE THE CARD. This logic might be too strong or based on a variant.
                      // Standard Cluedo: Only the suggester sees the card.
                      // For now, assuming this logic for observers knowing the shown card is intended for this game variant.
                      // If observers don't see the card, this else block shouldn't eliminate it for them.
                      // Let's assume they DO for now, as per prior code.
                      agent.memory.eliminateCard(challengeResult.cardToShow);
              ...
              // PASS THE PRIOR KNOWLEDGE SNAPSHOT
              const groundTruthDeductions = this.getDeterministicNewDeductions(agent, suggestingAgent, suggestion, challengeResult, eliminatedCardsBeforeThisTurnEvent);
   ...
   * @param {Set<string>} eliminatedCardsBeforeThisTurnEvent - Snapshot of agent.memory.eliminatedCards *before* current turn's pre-emptive updates.
   * @returns {Array<string>} - An array of card names newly deduced this turn.
   */
  getDeterministicNewDeductions(agent, suggestingAgent, suggestion, challengeResult, eliminatedCardsBeforeThisTurnEvent) {
      ...
      this.logger.debug(`Input state:
        ...
        - Eliminated cards (before this turn's event impact): ${Array.from(eliminatedCardsBeforeThisTurnEvent).join(', ')}
        - Eliminated cards (current in memory, incl. pre-emptive update): ${Array.from(agent.memory.eliminatedCards).join(', ')}`);

      // This set represents all cards known to the agent *before* the specific event being analyzed by Rules 1-3 
      // could have added new knowledge *for this turn*.
      const trulyKnownBeforeThisTurnEventAndHand = new Set([...eliminatedCardsBeforeThisTurnEvent, ...agent.cards]);

      // This set is for internal consistency within this function, reflecting knowledge *including* any pre-emptive update from this turn.
      const agentKnownCardsCurrent = new Set([...agent.memory.eliminatedCards, ...agent.cards]);
      ...
      if (isFirstUpdate) {
          ...
          for (const card of agent.cards) {
              // Own cards are always "newly deduced" on the first pass.
              this.logger.debug(`Rule 0: Adding own card ${card} as newly deduced`);
              newlyDeducedCards.add(card);
              // agentKnownCardsCurrent already includes agent.cards from its initialization.
          }
      } 
      // Note: On subsequent turns, own cards are already in trulyKnownBeforeThisTurnEventAndHand,
      // so they won't be added by other rules if those rules check against it.
      ...
      // Rule 1: If this agent is the suggester and a card was shown to them, they can deduce it *if it's truly new knowledge*
      if (agent.name === suggestingAgent.name && !challengeResult.noChallenge && challengeResult.cardToShow) {
          ...
          // To be a NEW deduction for this turn: not in hand AND not known before this turn's specific reveal.
          if (!agent.cards.has(shownCard) && !eliminatedCardsBeforeThisTurnEvent.has(shownCard)) {
              this.logger.debug(`Rule 1: ${agent.name} was shown ${shownCard}. Not in hand and not known from prior turns. Adding to newlyDeducedCards.`);
              newlyDeducedCards.add(shownCard);
          }
          // agentKnownCardsCurrent is updated for consistency if other rules depend on the current state of all known cards.
          // It already includes shownCard if it was pre-emptively added to agent.memory.eliminatedCards.
      }
      
      // Rule 2: If this agent is the suggester and they hold 2 of the 3 suggested cards,
      // and someone challenged, they can deduce the third card *if it's truly new knowledge*.
      if (agent.name === suggestingAgent.name && !challengeResult.noChallenge) {
          ...
          if (cardsAgentHolds.length === 2) {
              ...
              // To be a NEW deduction: not in hand (implicit by find) AND not known before this turn's specific reveal.
              if (deducedCard && !eliminatedCardsBeforeThisTurnEvent.has(deducedCard)) {
                  this.logger.debug(`Rule 2: ${agent.name} holds 2/3 cards. Deduced ${deducedCard}. Not known from prior turns. Adding to newlyDeducedCards.`);
                  newlyDeducedCards.add(deducedCard);
              }
          }
      }
        
      // Rule 3: If this agent is the challenger and showed a card.
      // This rule should NOT add to newlyDeducedCards. The card shown is from their own hand.
      // Its "first-time deduction" status (as an own card) is handled by Rule 0.
      if (agent.name === challengeResult.challengingAgent && challengeResult.cardToShow) {
          const shownCardByChallenger = challengeResult.cardToShow;
          // Sanity check: this card must be in their hand.
          if (!agent.cards.has(shownCardByChallenger)) {
             this.logger.warn(`Rule 3 Warning: Challenger ${agent.name} supposedly showed ${shownCardByChallenger}, but it's not in their hand.`);
          }
          // No addition to newlyDeducedCards here.
          this.logger.debug(`Rule 3: ${agent.name} challenged and showed ${shownCardByChallenger}. This is an own card, not added to newlyDeducedCards by this rule.`);
      }

      const finalDeductions = Array.from(newlyDeducedCards);
      this.logger.debug(`Final deductions for ${agent.name} (truly new this turn): ${finalDeductions.join(', ') || 'None'}`);
